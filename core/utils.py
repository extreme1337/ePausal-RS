...
def generate_invoice_doc(faktura, korisnik):
    """Generiši Word fakturu kao HTML"""
    html = f"""<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Faktura {faktura.broj}</title>
    <style>
        @page {{ size: A4; margin: 2cm; }}
        body {{ font-family: Arial; font-size: 12pt; line-height: 1.4; }}
...
<body>
    <div class="header">
        <div class="company-name">{korisnik.ime}</div>
        <div>Paušalno oporezivanje<br>Email: {korisnik.user.email}<br>JIB: {korisnik.jib}<br>Račun: {korisnik.racun}</div>
    </div>
    <div class="invoice-title">FAKTURA / INVOICE No. {faktura.broj}</div>
    <div style="text-align: right;">Invoice date: {faktura.datum}<br>Invoicing place: Republika Srpska, BiH</div>
    <div style="margin: 30px 0;"><strong>Kupac / Client:</strong><br>{faktura.klijent}</div>
    <table class="items-table">
        ...
        <tbody>
            <tr>
                ...
                <td>{faktura.opis or 'Programming services'}</td>
                <td style="text-align: center">unit</td>
                <td style="text-align: center">1</td>
                <td style="text-align: right">{faktura.iznos} KM</td>
                <td style="text-align: right">{faktura.iznos} KM</td>
            </tr>
            <tr class="total-row">
                <td colspan="5" style="text-align: right">Total price in KM:</td>
                <td style="text-align: right">{faktura.iznos} KM</td>
            </tr>
        </tbody>
    </table>
    <div style="text-align: right; margin-top: 50px;">
        {korisnik.ime}:<br>
        <div style="display: inline-block; width: 200px; border-bottom: 1px solid; margin-top: 40px;">M.P.</div>
    </div>
</body>
</html>"""
    
    filename = f'faktura-{faktura.broj.replace("/", "-")}.html'
    return ContentFile(html.encode('utf-8'), name=filename)
...
def generate_payment_slip_png(uplatnica, korisnik):
    ...
    # LIJEVA STRANA
    draw.text((25, 38), 'Uplatio je (ime, adresa i telefon)', fill='black', font=font_small)
    ...
    draw.text((598, 175), 'samo za uplate javnih prihoda', fill='black', font=font_italic)
    ...
    for i in range(count):
        ...
        if char and char != ' ':
            try:
                ...
                draw.text((box_x + width//2 - 5, y + 5), char, fill='black', font=font)
...
def get_chart_data_prihodi(korisnik):
    ...
    data_neto = [
        float(p.iznos - (p.iznos * Decimal('0.02')) - (Decimal(str(settings.PROSJECNA_BRUTO_PLATA)) * Decimal('0.70')))
        for p in prihodi
    ]
...
def process_uploaded_pdf(uploaded_doc):
    ...
    try:
        ...
        datum = f"{datum_match.group(3)}-{datum_match.group(2)}-{datum_match.group(1)}" if datum_match else None
        
        # Extract klijent
        lines = extracted_text.split('\n')
        klijent = next((line.strip() for line in lines if line.strip() and not line.strip().isdigit()), None)
...
"""
    ...
    try:
        send_mail(subject, message, settings.DEFAULT_FROM_EMAIL, [korisnik.user.email], fail_silently=False)
...
def log_audit(user, model_name, object_id, action, old_value=None, new_value=None, request=None):
...
def check_rate_limit(user, action, limit=10, period_minutes=60):
    ...
    if recent_actions >= limit:
        return False, f"Rate limit exceeded: maksimalno {limit} akcija u {period_minutes} minuta"
